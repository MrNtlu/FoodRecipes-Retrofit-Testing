...
import com.mrntlu.foodrecipes.AppExecutors;
...
public class RecipeApiClient {
    ...
    public void searchRecipesApi(String query, int pageNumber){
        ...
        AppExecutors.getInstance().networkIO().schedule(new Runnable() {
            ...
            public void run() {
                ...
                Log.d(TAG, "run: canceled due to timeout");
                handler.cancel(true);
            }
    ...
    private class RetrieveRecipesRunnable implements Runnable{
        ...
        public void run() {
            try {
                Response response=getRescipes(query,pageNumber).execute();
                if (cancelRequest){
                    return;
                }
        ...
        private Call<RecipeSearchResponse> getRescipes(String query, int pageNumber){
    ...
    private class RetrieveRecipeRunnable implements Runnable{
        ...
        public void run() {
            try {
                Response response=getRescipe(recipeId).execute();
                if (cancelRequest){
                    return;
                }
                if (response.code()==200){
        ...
        private Call<RecipeResponse> getRescipe(String recipeId){
